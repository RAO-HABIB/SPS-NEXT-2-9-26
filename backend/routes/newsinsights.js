...
const table = 'newsinsights';
const sectionName = 'newsinsights';
...
router.post('/items', authMiddleware, (req, res) => {
  const { category, title, href, image, date, readTime, featured, order_index } = req.body;
  try {
    const info = db.prepare(`
      INSERT INTO ${table} (category, title, href, image, date, readTime, featured, order_index)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(category, title, href, image, date, readTime, featured ? 1 : 0, order_index || 0);
...
router.put('/items/:id', authMiddleware, (req, res) => {
  ...
  try {
    db.prepare(`
      UPDATE ${table} 
      SET category = ?, title = ?, href = ?, image = ?, date = ?, readTime = ?, featured = ?, order_index = ? 
      WHERE id = ?
    `).run(category, title, href, image, date, readTime, featured ? 1 : 0, order_index || 0, req.params.id);
    res.json({ message: 'Updated successfully' });
  } catch (error) {
    res.status(500).json({ error: 'Database error' });
  }
