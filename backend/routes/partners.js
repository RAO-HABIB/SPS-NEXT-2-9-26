...
router.post('/items', authMiddleware, (req, res) => {
  const { name, logo, tagline, description, category, href, order_index } = req.body;
  try {
    const info = db.prepare(`
      INSERT INTO ${table} (name, logo, tagline, description, category, href, order_index)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(name, logo, tagline, description, category, href, order_index || 0);
...
router.put('/items/:id', authMiddleware, (req, res) => {
  ...
  try {
    db.prepare(`
      UPDATE ${table} 
      SET name = ?, logo = ?, tagline = ?, description = ?, category = ?, href = ?, order_index = ? 
      WHERE id = ?
    `).run(name, logo, tagline, description, category, href, order_index || 0, req.params.id);
    res.json({ message: 'Updated successfully' });
  } catch (error) {
    res.status(500).json({ error: 'Database error' });
  }
