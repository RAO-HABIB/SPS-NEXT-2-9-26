...
router.post('/items', authMiddleware, (req, res) => {
  const { name, description, image, href, order_index } = req.body;
  try {
    const info = db.prepare(`
      INSERT INTO ${table} (name, description, image, href, order_index)
      VALUES (?, ?, ?, ?, ?)
    `).run(name, description, image, href, order_index || 0);
...
router.put('/items/:id', authMiddleware, (req, res) => {
  ...
  try {
    db.prepare(`
      UPDATE ${table} 
      SET name = ?, description = ?, image = ?, href = ?, order_index = ? 
      WHERE id = ?
    `).run(name, description, image, href, order_index || 0, req.params.id);
    res.json({ message: 'Updated successfully' });
  } catch (error) {
    res.status(500).json({ error: 'Database error' });
  }
