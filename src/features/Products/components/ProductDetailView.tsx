...
import OverviewFeaturedTech from "./OverviewFeaturedTech";
import OverviewWhyChoose from "./OverviewWhyChoose";
import OverviewMetrics from "./OverviewMetrics";
import OverviewTestimonials from "./OverviewTestimonials";
  ...
  return (
    <main className="w-full bg-[#F4F7FA] min-h-screen">
      ...
      <section className="relative w-full bg-[#031B3D]">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-32 pb-24 sm:pt-36 sm:pb-32 lg:pt-44 lg:pb-40 relative z-10 text-center flex flex-col items-center justify-center min-h-[400px] sm:min-h-[450px]">
          ...
          <div className="flex items-center justify-center gap-3 text-sm font-medium text-slate-300">
            ...
            {data.slug === "all" ? (
              <span className="text-[#00a7e1]">Products</span>
            ) : (
              <>
                <Link href="/products" className="hover:text-[#00a7e1] transition-colors">Products</Link>
                <IconifyIcon icon="lucide:chevrons-right" width={16} className="text-slate-500" />
      ...
      {data.slug === "all" ? (
        ...
        <>
          {/* 2. Featured Technology Solutions */}
            ...
            <section className="py-24 sm:py-32 bg-white relative z-10">
              <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div className="text-center max-w-3xl mx-auto mb-16">
                  ...
                  <p className="text-slate-600 text-lg">Explore industry-leading products empowering organizations to thrive.</p>
                </div>
                  ...
                  {data.featuredSolutions.items.map((item, idx) => (
                    <div key={idx} className="group flex flex-col bg-slate-50 rounded-2xl overflow-hidden border border-slate-100 hover:shadow-xl transition-all hover:-translate-y-1 duration-300">
                      ...
                      <div className="p-6 flex flex-col flex-1">
                        ...
                        <Link href="#" className="inline-flex items-center gap-2 text-sm font-bold text-[#0057B8] hover:text-[#00a7e1] transition-colors mt-auto">
                          Learn More <IconifyIcon icon="lucide:arrow-right" width={16} />
                        </Link>
                      </div>
                    </div>
                  ))}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
                  ...
                  <div className="relative w-full max-w-md mx-auto lg:max-w-full">
                    ...
                    <div className="absolute -bottom-6 -right-6 sm:bottom-10 sm:-right-10 bg-[#00a7e1] text-white p-6 sm:p-8 rounded-3xl shadow-2xl flex flex-col items-center justify-center text-center transform rotate-3">
                      <span className="text-3xl sm:text-4xl font-black mb-1 leading-none">{data.whyChoose.badgeText.split('\n')[0]}</span>
                      <span className="text-xs sm:text-sm font-bold uppercase tracking-wider">{data.whyChoose.badgeText.split('\n')[1] || "Experience"}</span>
                    </div>
                  </div>
                      ...
                      {data.whyChoose.points.map((point, idx) => (
                        <div key={idx} className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:bg-white/10 transition-colors">
                          <div className="w-10 h-10 rounded-full bg-[#00a7e1] flex items-center justify-center mb-4 shadow-lg shadow-[#00a7e1]/20">
                            <IconifyIcon icon="lucide:check" width={20} className="text-white" />
                          </div>
            ...
            <section className="py-24 sm:py-32 bg-white">
              <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div className="text-center max-w-3xl mx-auto mb-16">
                  ...
                  <p className="text-slate-600 text-lg">Scalable plans built for businesses of all sizes.</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
                  {data.pricing.plans.map((plan, idx) => (
                    <div key={idx} className={`relative flex flex-col rounded-3xl p-8 sm:p-10 transition-transform hover:-translate-y-2 duration-300 ${plan.isPopular ? 'bg-[#031B3D] text-white shadow-2xl scale-105 z-10 border-2 border-[#00a7e1]' : 'bg-white text-[#031B3D] border border-slate-200 shadow-xl'}`}>
                      {plan.isPopular && (
                      ...
                      <h3 className={`text-xl font-bold mb-4 ${plan.isPopular ? 'text-white' : 'text-[#031B3D]'}`}>{plan.name}</h3>
                      <div className="flex items-end gap-1 mb-8">
                        <span className={`text-4xl sm:text-5xl font-black ${plan.isPopular ? 'text-white' : 'text-[#031B3D]'}`}>{plan.price}</span>
                        {plan.price !== "Free" && <span className={`text-sm font-semibold mb-1 ${plan.isPopular ? 'text-slate-400' : 'text-slate-500'}`}>/month</span>}
                      </div>

                      <button className={`w-full py-3.5 rounded-xl font-bold mb-8 transition-colors ${plan.isPopular ? 'bg-[#00a7e1] hover:bg-[#008dbf] text-white' : 'bg-[#F4F7FA] hover:bg-slate-200 text-[#031B3D]'}`}>
                        Get Started
                      </button>
                      ...
                      <ul className="flex flex-col gap-4 mt-auto">
                        {plan.features.map((feature, fIdx) => (
                          <li key={fIdx} className="flex items-center gap-3">
                            <div className={`flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center ${feature.included ? 'bg-[#00a7e1]/20 text-[#00a7e1]' : 'bg-transparent text-slate-300'}`}>
                              {feature.included ? (
                                <IconifyIcon icon="lucide:check" width={14} className={plan.isPopular ? "text-[#00a7e1]" : ""} />
                              ) : (
                                <IconifyIcon icon="lucide:minus" width={14} className={plan.isPopular ? "text-slate-500" : ""} />
                              )}
                            </div>
                            <span className={`text-sm font-medium ${!feature.included && (plan.isPopular ? 'text-slate-500' : 'text-slate-400')} ${feature.included && (plan.isPopular ? 'text-slate-200' : 'text-slate-600')}`}>
                              {feature.name}
                            </span>
                          </li>
                        ))}
                ...
                <div className="relative rounded-[3rem] overflow-hidden bg-[#031B3D] flex flex-col md:flex-row items-center justify-between p-10 sm:p-16 shadow-2xl">
                  ...
                  <div className="relative z-10 max-w-lg mb-10 md:mb-0">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                      <button className="bg-[#00a7e1] hover:bg-[#008dbf] text-white px-8 py-3.5 rounded-full font-bold shadow-lg transition-all hover:scale-105 flex items-center justify-center gap-2">
                        Request a Demo
                      </button>
                      <button className="bg-white/10 hover:bg-white/20 text-white border border-white/20 px-8 py-3.5 rounded-full font-bold transition-all flex items-center justify-center gap-2">
                        Contact Sales
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="relative z-10 w-full md:w-1/2 flex justify-end">
                    {/* Tech elements or illustration could go here */}
