...
/* =========================================================================
   Our Services for Government (Tabs & Sub-items as shown in reference)
   ========================================================================= */

interface GovernmentSubService {
  id: string;
  title: string;
  description: string;
}

interface GovernmentPracticeTab {
  id: string;
  label: string;
  icon: React.ComponentType<{ className?: string }>;
  services: GovernmentSubService[];
}

const GOVERNMENT_PRACTICE_TABS: GovernmentPracticeTab[] = [
  {
    id: "cybersecurity",
    label: "Cybersecurity",
    icon: ShieldCheck,
    services: [
      ...
      },
      {
        ...
        description:
          "Accelerate enterprise security operations with a structured 30-day onboarding blueprint, architecture review, and zero-trust policy implementation designed for municipal and federal environments.",
      },
      {
    ...
    services: [
      {
        ...
        description:
          "Deploy 24/7 multilingual generative AI chatbots that resolve constituent permit queries, tax inquiries, and 311 service tickets with verified government data sources.",
      },
      {
        id: "document-processing",
        title: "Intelligent Document Automation",
        description:
          "Automate county and municipal clerk recording, property deed indexing, and public records redaction using specialized computer vision and OCR pipelines.",
      },
      {
    ...
    services: [
      {
        ...
        description:
          "Compliant productivity environments enabling seamless, secure collaboration across remote personnel, field inspectors, and administrative offices.",
      },
      {
        id: "emergency-comms",
        title: "Unified Emergency Communications",
        description:
          "Interoperable real-time communication bridges connecting police, fire, EMS, public health, and agency executives during civic emergencies and natural disasters.",
      },
      {
        id: "civic-streaming",
    ...
    services: [
      {
        ...
        description:
          "SpinnLabs incubation environments enabling government agencies to safely prototype emerging civic technologies, blockchain records, and AI pilots before wide deployment.",
      },
      {
        id: "smart-county-iot",
        title: "Smart Government IoT Prototyping",
        description:
      ...
      },
      {
        ...
        description:
          "Hands-on immersion programs and specialized workshops training agency staff in modern cyber resilience, cloud management, and applied artificial intelligence.",
      },
    ],
  },
];
...
/* =========================================================================
   Featured Enterprise & Public Sector Customers (Matching CountyGovernment)
   ========================================================================= */
...
/* =========================================================================
   Main Component: GovernmentLayout
   ========================================================================= */

export default function GovernmentLayout() {
  const [activePracticeTabId, setActivePracticeTabId] = useState("cloud");
  const [activeServiceId, setActiveServiceId] = useState("migrate-vmware");
  ...
  const currentPractice =
    GOVERNMENT_PRACTICE_TABS.find((t) => t.id === activePracticeTabId) ||
    GOVERNMENT_PRACTICE_TABS[0];
  const currentService =
    currentPractice.services.find((s) => s.id === activeServiceId) ||
    currentPractice.services[0];

  const handlePracticeTabChange = (tabId: string) => {
    setActivePracticeTabId(tabId);
    const targetPractice = GOVERNMENT_PRACTICE_TABS.find((t) => t.id === tabId);
    if (targetPractice && targetPractice.services.length > 0) {
      setActiveServiceId(targetPractice.services[0].id);
    }
  };
  ...
  return (
    <main className="w-full min-h-screen bg-[#F8F9FB] text-slate-900 selection:bg-[#00a7e1] selection:text-white">
      {/* 1. HERO SECTION WITH CAPITOL DOME & "OUR SOLUTIONS" CARD (Exact Match to User Screenshot) */}
      <section className="relative w-full min-h-[480px] sm:min-h-[540px] pt-32 sm:pt-36 lg:pt-40 pb-20 sm:pb-24 flex items-center overflow-hidden bg-[#091E36]">
        {/* Stylized Architectural Capitol Dome Silhouette in Center-Right */}
        <div className="absolute inset-0 pointer-events-none z-0 flex items-center justify-center lg:justify-end overflow-hidden">
          <div className="relative w-[560px] h-[660px] opacity-35 translate-x-12 sm:translate-x-8 lg:translate-x-0">
            <Image
              src="/images/verticals/capitol-dome.svg"
              alt="Capitol Dome Silhouette"
              fill
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-[#091E36] via-[#091E36]/90 to-transparent z-0 pointer-events-none" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-center">
            ...
            <div className="lg:col-span-7">
              ...
              <p className="text-slate-200/90 text-sm sm:text-base leading-relaxed font-normal">
                Governments continue to build on their momentum to deliver exceptional citizen services —
                and strengthen their readiness in advance of the next big disruptions. In support, SPS
                remains committed to the service of governments and citizens around the globe — across
                segments including social services, government health, tax management, critical
                infrastructure, education, and national security. We work with stewards in federal as well
                as state and local agencies, building on decades of experience and trust. On a foundation
                of security and automation, our industry solutions help governments improve the citizen
                experience, optimize operations, and modernize infrastructure.
              </p>
            </div>
            ...
            <div className="lg:col-span-5">
              <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-6 sm:p-7 shadow-2xl">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <button
                    onClick={() => handleOpenModal("Government Solutions Overview")}
      ...
      {/* 2. FRAMER PILL DROPDOWN NAV (Framer Style: Pill-Dropdown-Nav, positioned exactly like CountyGovernment) */}
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      {isActive && (
                        <motion.div
                          layoutId="activeVerticalPillGov"
      ...
      {/* 3. OUR SERVICES FOR GOVERNMENT SECTION (Interactive Tabs - Chrome Fluid Tab Bar + White Card Split View) */}
      <section className="py-14 sm:py-18 lg:py-20 bg-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          {/* Section Header */}
          <div className="text-center max-w-3xl mx-auto mb-10 sm:mb-12">
            <h2 className="text-3xl sm:text-4xl font-extrabold text-[#031B3D] tracking-tight mb-2 sm:mb-3">
          ...
          <div className="bg-white rounded-3xl border border-slate-200/90 shadow-xl overflow-hidden">
            {/* Top Tab Bar with Inverted Curved Wings (Image 3 Style) */}
            <div className="bg-[#F3F5F9] border-b border-slate-200/90 px-4 sm:px-8 pt-3 flex items-end gap-1 sm:gap-2 overflow-x-auto no-scrollbar">
              {GOVERNMENT_PRACTICE_TABS.map((tab) => {
                const isActive = tab.id === activePracticeTabId;
                const TabIcon = tab.icon;

                return (
                  <button
                    key={tab.id}
                    onClick={() => handlePracticeTabChange(tab.id)}
            ...
            {/* Sub-Tabs & Content Pane (Matching User Reference Image) */}
            <div className="p-6 sm:p-10 lg:p-12">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
                {/* Left Column: Vertical Sub-tabs */}
                <div className="lg:col-span-4 flex flex-col gap-2">
                  {currentPractice.services.map((svc) => {
      ...
      {/* 4. CUSTOMERS WE ARE PROUD TO WORK WITH (Matching CountyGovernmentLayout Round Circles) */}
      <section className="py-20 sm:py-28 bg-[#F8F9FB] text-slate-900 border-t border-b border-slate-200/80">
        <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-center">
          ...
          {/* Circular Customer Badges (Exact NetworkSecurityLayout & CountyGovernment round circle style) */}
      ...
      <RequestQuoteModal
        ...
        serviceTitle={`Government - ${selectedSolutionTitle || currentService.title}`}
      />
    </main>
  );
}
